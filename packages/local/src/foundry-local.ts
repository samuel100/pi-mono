...
/**
 * Minimal type surface for the Foundry Local SDK manager.
 * The SDK ships without .d.ts, so we define only what we use.
 */
interface FLSdkManager {
	readonly isWebServiceRunning: boolean;
	readonly urls: string[];
	startWebService(): void;
	stopWebService(): void;
	catalog: FLSdkCatalog;
}

interface FLSdkCatalog {
	getModels(): Promise<FLSdkModel[]>;
	getModel(alias: string): Promise<FLSdkModel>;
	getLoadedModels(): Promise<FLSdkModel[]>;
	lastFetch?: number;
}

interface FLSdkModel {
	id?: string;
	alias: string;
	isCached: boolean;
	supportsToolCalling?: boolean;
	contextLength?: number;
	info?: { fileSizeMb?: number; maxOutputTokens?: number };
...
let FLManagerClass: { create: (opts: { appName: string; logLevel: string }) => FLSdkManager } | null = null;
try {
	FLManagerClass = cjsRequire("foundry-local-sdk").FoundryLocalManager;
} catch {
	// SDK not installed on this platform -- isAvailable() will return false
}

export class FoundryLocalManager {
	private sdkAvailable: boolean | null = null;
	private sdkManager: FLSdkManager | null = null;
	...
	isAvailable(): boolean {
		...
		this.sdkAvailable = FLManagerClass !== null;
	 ...
	 */
	private async getOrCreateManager(): Promise<FLSdkManager> {
		if (this.sdkManager) return this.sdkManager;
		if (!FLManagerClass) throw new Error("Foundry Local SDK is not available");

		// Windows: skip POSIX fd manipulation — no /dev/null, no dup/dup2
		if (process.platform === "win32") {
			this.sdkManager = FLManagerClass.create({
		...
		// Unix: suppress stdout during SDK init to prevent noise
		let savedFd = -1;
