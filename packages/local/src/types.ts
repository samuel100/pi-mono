 ...
 * (Foundry Local, Ollama, LM Studio, etc.) to plug into any consumer app.
 * They use neutral types — no dependency on pi-ai or any specific consumer.
 */

/**
 * Neutral model descriptor returned by local providers during discovery.
 * Consumer apps translate these into their own model representations.
 */
 ...
 * Implement this to plug a new local runtime into Pi.
 *
 * @example
 * // Foundry Local
...
// These are used by UI components (model-selector, etc.) to interact
// with LifecycleManager without depending on specific provider details.
...
/** Consumer-facing lifecycle operations for local model providers. */
export interface ModelLifecycleProvider {
	/** True if any local provider is registered and available. */
	hasProviders(): boolean;

	/** Ensure all registered providers have discovered their models. Lazy — safe to call repeatedly. */
	...
	download(provider: string, modelId: string, onProgress: (percent: number) => void): Promise<void>;

	/** Prepare a model for streaming. Returns baseUrl override if applicable. */
	prepare(provider: string, modelId: string): Promise<{ baseUrl: string } | undefined>;

	/** Dispose all providers. Call on session shutdown. */
	dispose(): Promise<void>;
}
